...
void main(){
   ...
   float lightType = (g_LightColor.a == 5.0) ? 1.0 : g_LightColor.a;
   ...
   vec4 wvLightPos = (g_ViewMatrix * vec4(g_LightPosition.xyz,clamp(lightType,0.0,1.0)));
...
//   lightVec.w = fract(g_LightDirection.w);
   ...
   lightColor.w = 1.0;
    ...
    if (g_LightColor.a != 5.0) {
        lightDistance = vec2(-5000.0, -5000.0);
    } else {
        lightDistance = wvLightPos.xy + vec2(0.0, 7.0)- wvPosition.xy;
    }
    SpecularBrightness = ((m_Specular.x / m_Diffuse.x) + (m_Specular.y / m_Diffuse.y) + (m_Specular.z / m_Diffuse.z))/3.0;
}